...
# =============================================================================
Energy = False
energy_cut = 0.2									# Reading from .boltzmann (kcal/mol)
molecules_cut = 5                           
sh_files = True
...
for item in out_files:  
    test_out.append(item.split('.')[0])

if test_boltzmann==test_out:
    print('Los archivos .out y .boltzmann son correctos\n')
else:
    exit('Problemas con los archivos .out y/o .boltzmann. No hay la misma cantidad o tienen diferentes nombres.')
    ...
    while True:
        ...
        if '-------------------------------------------------------------------------------' in l:
            while True:
                ...
                if 'CELL' in k: 
                ...
                elif 'CARTESIAN COORDINATES' in k and cell in to_extract:
                    ignoreLines(f, 1)
                    cont = 0
                    chunk = []        
                    while cont < natoms:
                    ...
                    with open(out.split('.')[0]+'_Cell_'+str(cell)+'.gjf', 'wt') as final:
                        final.write('%chk={}.chk\n'.format(out.split('.')[0]+'_Cell_'+str(cell)))
                        final.write(mem+'\n')                        #
                        final.write(nproc+'\n')                      #
                        ...
                        final.write('\n ')
                        print(out.split('.')[0]+'_Cell_'+str(cell))
